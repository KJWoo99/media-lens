"""Image Search page - CLIP-based text-to-image semantic search."""
...
from PyQt6.QtWidgets import (
    ...
    QFrame, QSlider, QSizePolicy, QCheckBox, QMenu, QApplication, QFileDialog,
)
...
class ModelLoadThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            backend = "TRT FP16" if self.engine.trt_vision else "PyTorch"
            self.finished.emit(True, f"CLIP ready ({self.engine.device}, {backend})")
        except BaseException as e:
            _log.getLogger(__name__).error(
                f"CLIP ModelLoadThread crashed: {type(e).__name__}: {e}\n"
                + _tb.format_exc())
            try:
...
class ImageSearchPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.model_status.setStyleSheet(f"font-size: 12px; color: {COLORS['text3']};")
        ...
        self.folder_picker = FolderPicker(
            ...
            config_key="clip_search")
        self.folder_picker.folder_changed.connect(lambda _: self._load_folder())
        ...
        self.load_status.setStyleSheet(f"font-size: 12px; color: {COLORS['text2']};")
        ...
        self.search_input.setPlaceholderText("Search images by text (Korean / English)...")
        self.search_input.setFixedHeight(38)
        self.search_input.setStyleSheet(
            f"font-size: 14px; border-radius: 10px; padding: 8px 14px;")
        ...
        self.count_slider.valueChanged.connect(lambda v: self.count_label.setText(str(v)))
        ...
        self.search_info.setStyleSheet(f"font-size: 12px; color: {COLORS['text2']};")
    ...
    def set_engine(self, engine):
        ...
        self.model_status.setText("Loading CLIP model...")
        self.model_status.setStyleSheet(f"font-size: 12px; color: {COLORS['yellow']};")
    ...
    def _load_folder(self):
        ...
        # Stop any previous load cleanly
    ...
    def _on_load_stopped(self):
        ...
        self.load_status.setStyleSheet(f"font-size: 12px; color: {COLORS['text3']};")
    ...
    def _on_load_done(self, count):
        ...
        self.load_status.setStyleSheet(f"font-size: 12px; color: {COLORS['green']};")
    ...
    def _on_load_error(self, msg):
        ...
        self.load_status.setStyleSheet(f"font-size: 12px; color: {COLORS['red']};")
    ...
    def _apply_score_filter(self):
        ...
        for i, (path, score) in enumerate(filtered):
            ...
            score_color = (COLORS['green'] if score > 0.85
                           else COLORS['accent'] if score > 0.7
                           else COLORS['text2'])
